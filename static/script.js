...
/**
 * Show error section with retry button
...
async function processChannel(channelUrl) {
    try {
        ...
        const response = await fetch('/process_channel', {
            ...
            // Extended timeout for initial processing request (channels can take time to scan)
        ...
        const errorMsg = error.name === 'TimeoutError' 
            ? 'Timeout: O servidor está demorando para responder. Tente novamente.'
            : 'Erro na comunicação com o servidor. Verifique sua conexão.';
        showError(errorMsg);
    }
}
...
async function processVideo(videoUrl) {
    try {
        ...
        const response = await fetch('/process', {
            ...
            // Extended timeout for initial processing request
            signal: AbortSignal.timeout(120000) // 2 minute timeout
        });
...
/**
 * Get adaptive timeout based on progress and retry attempts
 * @param {number} progress - Current progress percentage (0-100)
 * @param {number} retryCount - Number of retry attempts
 * @returns {number} Timeout in milliseconds
 */
function getAdaptiveTimeout(progress, retryCount = 0) {
    // Base timeout increases with retry attempts
    let baseTimeout = 30000; // 30 seconds base
    
    // Increase timeout based on progress stage
    if (progress < 10) {
        // Initial stage: scanning/setup can take longer
        baseTimeout = 60000; // 1 minute
    } else if (progress < 50) {
        // Download stage: depends on video length
        baseTimeout = 90000; // 1.5 minutes
    } else if (progress < 90) {
        // Processing stage: can be intensive
        baseTimeout = 120000; // 2 minutes
    } else {
        // Final stage: upload/cleanup
        baseTimeout = 60000; // 1 minute
    }
    
    // Add extra time for each retry (exponential backoff on timeout too)
    const retryMultiplier = Math.pow(1.5, retryCount);
    
    // Cap maximum timeout at 5 minutes to avoid hanging indefinitely
    return Math.min(baseTimeout * retryMultiplier, 300000);
}
...
async function monitorJob(jobId, type = 'video') {
    ...
    let currentProgress = 0; // Track current progress for adaptive timeout
    ...
    const checkStatus = async () => {
        try {
            const response = await fetch(`/status/${jobId}`, {
                ...
                // Adaptive timeout based on current progress and retry count
                signal: AbortSignal.timeout(getAdaptiveTimeout(currentProgress, retryCount))
            });
            ...
            // Update current progress for adaptive timeout
            ...
            if (retryCount < maxRetries) {
                ...
                if (typeof updateProgress === 'function') {
                    updateProgress('retrying', 0, `Reconectando... (tentativa ${retryCount}/${maxRetries})`);
                }
                
                // Exponential backoff: wait longer between retries
                const retryDelay = Math.min(3000 * retryCount, 15000); // Max 15 seconds
                ...
                // Show error section instead of just toast
                showError(errorMsg);
            }
        }
    };
